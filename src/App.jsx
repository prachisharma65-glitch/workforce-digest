...
const OPEN_QUESTIONS = [
 ...
 { q: "How much does action-level identification erode the team-level privacy commitment over time?", a: "Open. Non-goal #3 prevents browseable per-employee scores, but action items necessarily name individuals to line managers. Over months, line managers may build de facto per-employee surveillance models from accumulated alerts. System-level audit logging and escalation paths mitigate this without suppressing active risk signals; cool-down suppression was considered and rejected as it would hide active risk from the responsible manager. Empirical study with design partners is on the V2 roadmap." },
...
const NON_GOALS = [
  ...
  { title: "No browsable per-employee attrition scores", reason: "Team-level patterns surface to managers; individual evidence is visible only when a specific alert fires. Managers who can sort their reports by attrition risk make different (and worse) decisions than managers who get one-off intervention signals. Important honest disclosure: this is layered defense, not full protection. Action-level identification (e.g., \"assign Sasha as buddy for Jordan\") still names individuals to line managers by necessity. V2 mitigations to address the residual risk: system-level audit logging of per-employee alert frequency, surfaced to product fairness reviewers and HR ops (not to the line manager) — catches over-flagging at the system level without suppressing active risk; escalation paths when alerts persist across multiple weeks (raising visibility, not lowering it); manager-side context display showing prior actions taken when an alert repeats; line-manager training that frames alerts as point-in-time interventions. Deliberately NOT cool-down suppression — that would hide active risk from the responsible manager, which is the wrong trade." },
...
const NOISE_CONTROLS = [
  ...
  { title: "Per-employee audit logging (V2)", body: "Tracks how often each individual triggers alerts and across which signals. Surfaces over-flagging patterns at the system level — visible to product fairness reviewers and HR ops, NOT to the line manager. Catches surveillance-accumulation risk without suppressing active alerts from the responsible manager. Persistent alerts escalate upward (to department manager or HR) rather than being silenced. The system increases visibility on unresolving issues, never decreases it." },
];
